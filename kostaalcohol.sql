...
select p_code,cate_code,p_name,p_alcohol,p_price,p_stuck,p_date,p_detail from product order by p_code;
...
delete from users where u_id='sun';

select * from users

update users set u_addr='06536' where u_addr='서울시 마포구'
update users set u_addr2='서울특별시 서초구 강남대로 527' where u_addr2='안다미로'; 
update users set u_point=0 where ;
update users set u_addr3='1동 1호' where u_addr3='5동 1502호'
delete from users where u_addr3='1동 1호'
---------------------------------------------------------------------------------
...
create table product( --상품 
  ...
  p_price number, --상품가격
  p_stuck number , --재고수량 
  p_date varchar2(30) , --출시일  -----> date로 변경됨  
...
update product set p_alcohol='37' where p_alcohol='36.5' 
...
insert into subscription values('sub_' || subscription_no_seq.nextval,'min',to_date('2021/11/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'jung',to_date('2021/11/01','YY/MM/DD'),20000);

insert into subscription values('sub_' || subscription_no_seq.nextval,'lee',to_date('2021/12/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'min',to_date('2021/12/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'jung',to_date('2021/12/01','YY/MM/DD'),20000);

insert into subscription values('sub_' || subscription_no_seq.nextval,'min',to_date('2022/01/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'jung',to_date('2022/01/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'lee',to_date('2022/01/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'kim',to_date('2022/01/01','YY/MM/DD'),20000);

insert into subscription values('sub_' || subscription_no_seq.nextval,'min',to_date('2022/02/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'jung',to_date('2022/02/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'lee',to_date('2022/02/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'kim',to_date('2022/02/01','YY/MM/DD'),20000);
insert into subscription values('sub_' || subscription_no_seq.nextval,'sun',to_date('2022/02/01','YY/MM/DD'),20000);
...
insert into notice values(notice_no_seq.nextval,'admin','이달의 우리 술 출시','안녕하세요? 안다미로입니다. 안다미로는 
매달 각 지역의 전통술을 출시하고 있습니다. 많은 관심 부탁드립니다',to_date('2021/10/01','YY/MM/DD') ,default);

insert into notice values(notice_no_seq.nextval,'admin','신상품 출시','안녕하세요? 안다미로입니다. 
신상품이 출시되었습니다. 많은 관심 부탁드립니다',to_date('2021/11/15','YY/MM/DD') ,default);

insert into notice values(notice_no_seq.nextval,'admin','배송 가능 시간 안내','결제 완료 시 다음 영업일에 출고되며 공휴일의 경우
2일 후에 출고됩니다. 출고된 경우 서울/수도권 지역은 대부분 다음날 수령, 지방은 3일 이내에 배송됩니다.',to_date('2021/12/05','YY/MM/DD') ,default);

insert into notice values(notice_no_seq.nextval,'admin','택배사 파업 안내','현재 택배사 파업으로 인해 배송 지연되고 있는 점 안내드립니다.
주문 시 이 점 참고하여 주시길 바랍니다',to_date('2022/02/15','YY/MM/DD') ,default);
...
,to_date('2021/10/28','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'lee',1,'상품문의','술잔만 따로 구매하고 싶은데 가능한가요?'
,to_date('2021/11/01','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'kim',1,'상품문의','미니어처 제품 판매 할 계획이 없나요?'
,to_date('2021/11/10','YY/MM/DD') ,1);




insert into QA values(qa_no_seq.nextval,'jung',2,'결제/환불/교환 문의','주문한 제품과 다른 상품이 배송되었습니다. 교환해주세요',
to_date('2021/11/14','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'lee',2,'결제/환불/교환 문의','상품이 깨진상태로 배송되었는데 어떻게 해야하나요?',
to_date('2021/11/27','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'sun',2,'결제/환불/교환 문의','배송이 지연되고 있는데 상품이 언제쯤 출고되나요?',
to_date('2021/12/03','YY/MM/DD') ,1);
...
insert into QA values(qa_no_seq.nextval,'jung',3,'쿠폰/포인트 문의','포인트는 언제까지 사용가능한가요?'
,to_date('2022/01/15','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'min',3,'쿠폰/포인트 문의','쿠폰은 중복사용이 안되나요?'
,to_date('2022/02/06','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'lee',3,'쿠폰/포인트 문의','포인트는 얼마부터 사용 가능한가요?'
,to_date('2022/03/13','YY/MM/DD') ,1);
...
insert into QA values(qa_no_seq.nextval,'kim',4,'기타문의','전통술 외에 다른 종류는 판매하지 않는건가요?'
,to_date('2022/01/06','YY/MM/DD') ,1);

insert into QA values(qa_no_seq.nextval,'jung',4,'기타문의','상담원과 연락하고 싶은데 번호가 보이지 않아요'
,to_date('2022/02/18','YY/MM/DD') ,1);
...
,to_date('2021/10/29','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 lee님, 술잔은 따로 판매하고 있지 않습니다'
,to_date('2021/11/02','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 kim님, 현재 미니어처 제품 계획중에 있으며 추후 공지사항을 통해 안내해드리겠습니다'
,to_date('2021/11/11','YY/MM/DD') ,1);

commit

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 jung님 불편을 드려 죄송합니다. 담당자가 금일 연락드릴 예정입니다'
,to_date('2021/11/15','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 lee님 불편을 드려 죄송합니다. 담당자가 금일 연락드릴 예정이며 환불 또는 교환해드리겠습니다'
,to_date('2021/11/28','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 sun님 불편을 드려 죄송합니다. 현재 생산라인의 문제로 인해 2일후에 배송출고 될 예정이며 양해 부탁드립니다'
,to_date('2021/12/04','YY/MM/DD') ,1);


delete from reply where re_number=7

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 jung님, 포인트는 정해진 기간이 없이 사용 가능합니다'
,to_date('2022/01/16','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 min님, 쿠폰은 중복사용이 어려우며 포인트와는 중복사용이 가능합니다'
,to_date('2022/02/07','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 lee님, 포인트는 1p=1원으로 금액 상관없이 사용 가능합니다'
,to_date('2022/03/14','YY/MM/DD') ,1);




insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 kim님, 안다미로는 전통술만 판매하는 쇼핑몰입니다.
다른 종류는 판매 할 예정이 없습니다.',to_date('2022/01/07','YY/MM/DD') ,1);

insert into reply values(reply_no_seq.nextval,qa_no_seq.currval,'admin','안녕하세요 jung님, 연락받으실 번호 남겨주시면 빠른시일내에 상담원이
통화하도록 하겠습니다',to_date('2022/02/19','YY/MM/DD') ,1);
...
insert into review values(review_no_seq.nextval,'kim','C03','회와 같이 맛있게 먹었습니다. 설명처럼 해산물과 잘 어울려요',5,'img');
insert into review values(review_no_seq.nextval,'lee','M01','부모님께 선물해드렸는데 좋아하셨어요',5,'img');
insert into review values(review_no_seq.nextval,'min','A01','무화과맛이라서 여성분들이 좋아할거에요',3,'img');
insert into review values(review_no_seq.nextval,'jung','B05','제주도 여행가서 먹었었는데 맛있어서 온라인으로 찾아가 구매합니다',4,'img');
insert into review values(review_no_seq.nextval,'kim','B06','솔향기가 시원해서 좋아요',5,'img');
insert into review values(review_no_seq.nextval,'jung','D06','친구들과 파티위해 구매했는데 만족스러웠어요',5,'img');
insert into review values(review_no_seq.nextval,'lee','G06','가족들은 맛있다고 했는데 제 입맛에는 너무 달았어요',3,'img');
insert into review values(review_no_seq.nextval,'min','C06','자두맛 사탕을 좋아하시는 분들이라면 추천합니다',5,'img');
insert into review values(review_no_seq.nextval,'lee','B10','어느곳에나 어울리는 무난한 술 같아요',3,'img');
insert into review values(review_no_seq.nextval,'jung','A04','이름이 신기해서 구매했는데 제 입맛에는 별로였어요',2,'img');
insert into review values(review_no_seq.nextval,'kim','D09','매화수보다 맛있어요',4,'img');
insert into review values(review_no_seq.nextval,'min','G02','술 좋아하는 친구에게 선물할때 좋아요',4,'img');
insert into review values(review_no_seq.nextval,'kim','M04','제주도 여행이 생각나는 술이에요',5,'img');
